...
const SearchBar = forwardRef((props: SearchBarProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'group border-border-medium text-text-primary focus-within:bg-surface-hover hover:bg-surface-hover relative mt-1 flex h-10 cursor-pointer items-center gap-3 rounded-lg px-3 py-2 transition-colors duration-200',
        isSmallScreen === true ? 'mb-2 h-14 rounded-2xl' : '',
      )}
    >
      <Search className="text-text-secondary group-focus-within:text-text-primary group-hover:text-text-primary absolute left-3 h-4 w-4" />
      <input
        type="text"
        ref={inputRef}
